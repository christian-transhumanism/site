...
## July 1, 2026 Traffic-Scaling Push (bidding + keyword broadening)

Context: campaign was serving almost nothing — last-30-days `2` clicks / `50` impressions / `$3.83`, with a diagnostic "Your campaign hasn't generated clicks in the past week." Diagnosis from the keyword view: three compounding drags — (1) narrow exact/phrase keywords (low search volume), (2) low Quality Scores (many at 2-4/10; several keywords show "Rarely shown (low Quality Score)"), and (3) the Ad Grants $2 CPC cap (e.g. `brain computer interface` was "Below first page bid ($2.23)" — literally could not bid enough to appear).

Change 1 (the big lever) — switched `USA` bid strategy from Manual CPC (enhanced) to **Maximize Conversions**. Confirmed current Ad Grants rules first (Maximize Clicks keeps the $2 cap; only conversion-based Smart Bidding removes it). Maximize Conversions removes the $2 cap (can bid $4-12+ when a conversion is predicted) and is CTR-friendly (chases high-intent/clickier traffic). It uses the existing `membership_signup_intent` conversion action. After saving, campaign status shows `Bid strategy learning`, the optimization panel reads "Focused on conversions, based on your Maximize conversions bid strategy," and all keyword Max CPCs changed from `$2.00` to `—` (automated). Left the optional target CPA OFF (pure Max Conversions). Did NOT enable "AI Max" (broad-match auto-expansion — CTR risk we're avoiding). Caveat: with ~0 conversion history, Maximize Conversions can behave unpredictably at first (usually opens traffic by removing the cap; occasionally a no-data account bids timidly). Fully reversible (can switch to another strategy; note you cannot switch back to the deprecated Enhanced CPC specifically).

Change 2 — broadened `AI and Human Flourishing` with 7 phrase-match keywords (flourishing/meaning/dignity angle, distinct from the AI Ethics group's ethics/risk angle): `"human flourishing"`, `"ai and human dignity"`, `"technology and meaning"`, `"ai for good"`, `"technology and human values"`, `"purpose of technology"`, `"can technology make life better"`. (The 37 campaign-level negatives from 2026-06-29 already apply, so no per-group negatives needed.) This is the second group broadened after `AI Ethics and Future` (2026-06-29).

WATCH — account/campaign CTR risk: at the time of these changes the `USA` campaign showed ~`4.00%` CTR (2 clicks / 50 impr) for the period — BELOW the Ad Grants 5% threshold, though on a tiny/noisy sample. Maximize Conversions should push CTR up (it optimizes toward clickier converting traffic), and broadening on relevant phrase terms should add clicks with impressions. But monitor account-level monthly CTR closely over the next 1-2 weeks: if it trends below 5%, pause the weakest low-QS / low-CTR keywords (the `Science & Faith`, low-QS thinker exacts) and hold off on further broadening. Two consecutive months < 5% risks temporary Ad Grants deactivation.

Not done this session (deliberate stopping point): broadening the remaining topic groups (`Brain-Computer Interfaces`, `Radical Longevity`, `Dominion and Future Values`, `Technology and Human Flourishing`, `Human Enhancement Ethics`) — do these next after seeing how the bidding change + first two broadenings affect impressions/CTR. Also pending: an offered `Remove 13 redundant keywords` recommendation (reach-preserving exact->phrase consolidation; safe to apply).

Operational note: in the Add-Keywords panel, TYPE only after confirming focus is in the textarea — typing while unfocused triggers Google Ads single-key navigation shortcuts (it jumped to the Recommendations page once). Click the textarea, verify the cursor/first word landed, then continue.

## July 6, 2026 Maximize Conversions Reverted — Experiment Failed

Live check of `USA` (last 30 days, `Jun 6 – Jul 5`): **`0` clicks, `38` impressions, `0.00` conversions, `$0.00` cost.** Campaign state `Eligible (Limited)` — the Overview diagnostic read "Your campaign hasn't generated conversions in the past week / It is only eligible to serve to a limited audience," with a `Targeting fewer searches` warning under the `Audiences` tab. This is a sharp regression from the 6/29 prior-30-day figures (16 clicks / 260 impressions / $16.09): impressions fell ~85% and clicks/spend collapsed to zero.

Root cause: the 7/01 switch to **Maximize Conversions** backfired exactly as the "no-data account bids timidly" caveat warned. With ~zero conversion history the strategy had nothing to learn from, so instead of removing the $2-cap ceiling and opening traffic, Google throttled delivery to a "limited audience" and the campaign essentially stopped bidding. The theoretical CPC-cap benefit was moot because it wasn't winning (or entering) auctions at all.

**Action — reverted the same day.** `Campaign settings` -> `Budget and bidding optimization` -> `Bidding` -> `Change bid strategy` -> "What do you want to focus on?" = **Clicks** (= Maximize Clicks). Checked **"Set a maximum cost per click bid limit"** and entered **`$2.00`** (required for Ad Grant compliance — Maximize Clicks is NOT Smart Bidding, so the $2 cap applies). Google showed the expected info note "you've chosen to focus on clicks... but your account tracks conversions." Saved. After save the Bidding row read `Maximize clicks` and campaign status changed to `Bid strategy learning`. AI Max left OFF. The 7/01 keyword broadening was left in place.
